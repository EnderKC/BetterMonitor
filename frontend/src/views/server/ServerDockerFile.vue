...
const fetchFileList = async (path: string = '/') => {
  ...
  try {
    const response = await request.get(buildUrl('/files'), {
...
const fetchDirectoryTree = async () => {
  try {
    ...
    const response = await request.get(buildUrl('/files/children'), {
      params: { path: '/' }
    });
...
const openFileEditor = async (file: any) => {
  ...
  try {
    ...
    const response: string | ApiResponse = await request.get(buildUrl('/files/content'), {
...
const saveFileContent = async () => {
  ...
  try {
    ...
    await request.put(buildUrl('/files/content'), {
      path: filePath,
...
const downloadFile = (file: any) => {
  ...
  const downloadUrl = `${window.location.origin}/api/servers/${serverId.value}/docker/containers/${containerId.value}/files/download?path=${encodeURIComponent(filePath)}&token=${token}`;
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        ...
        await request.post(buildUrl('/files/delete'), { paths });
...
const createFileOrDirectory = async () => {
  ...
  try {
    ...
    if (createFormState.type === 'file') {
      ...
      await request.post(buildUrl('/files/mkdir'), { path });
      ...
      <div class="window-toolbar">
        <div class="toolbar-left">
          ...
          <div class="nav-controls">
            ...
            <span class="window-title">{{ containerName || '容器文件管理' }}</span>
          </div>
        </div>
