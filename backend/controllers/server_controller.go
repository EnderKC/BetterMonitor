...
func Heartbeat(c *gin.Context) {
	...
	if secretKey == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "未提供密钥"})
		return
	}
	...
	// 验证密钥
	if server.SecretKey != secretKey {
		// 密钥验证失败，返回错误但不更新心跳时间
		c.JSON(http.StatusUnauthorized, gin.H{"error": "密钥无效"})
		return
	}
	...
	if err := c.ShouldBindJSON(&heartbeatData); err == nil {
		...
		if heartbeatData.Status != "" {
			log.Printf("更新服务器 %d 状态为 %s (来自Agent心跳)", serverID, heartbeatData.Status)
			err = models.UpdateServerHeartbeatAndStatus(serverID, heartbeatData.Status)
			if err != nil {
		...
		// 更新Agent版本信息
		if heartbeatData.Version != "" {
			log.Printf("更新服务器 %d Agent版本为 %s", serverID, heartbeatData.Version)
			err = models.UpdateServerAgentVersion(serverID, heartbeatData.Version)
			if err != nil {
				log.Printf("更新Agent版本失败: %v", err)
			}
		}
	} else {
		// 如果没有提供请求体或解析失败，仍然更新状态为online
		log.Printf("心跳请求没有提供状态信息，默认更新为online (来自Agent心跳)")
		err = models.UpdateServerHeartbeatAndStatus(serverID, "online")
		if err != nil {
			log.Printf("更新服务器状态失败: %v", err)
		}
	}
